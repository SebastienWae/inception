...
if [ ! -e $DATA_DIR/swaegene.42.fr/wordpress ]; then
  ...
  gosu www-data wp core install --url="$URL" --title="$TITLE" --admin_user=$ADMIN_USER --admin_password=$ADMIN_PASS --admin_email=$ADMIN_EMAIL --skip-email
